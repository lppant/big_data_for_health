...
def predict_mortality(model, device, data_loader):
	...
	probas = []
	with torch.no_grad():
		for index, (input_feature, target_feature) in enumerate(data_loader):
			if isinstance(input_feature, tuple):
				input_feature = tuple([index.to(device) if type(index) == torch.Tensor else index for index in input_feature])
			else:
				input_feature = input_feature.to(device)
			output = model(input_feature)
			softmax_output = nn.Softmax()(output).numpy()
			selected_output = softmax_output[:, 1].tolist()
			probas.append(selected_output[0])
	return probas
